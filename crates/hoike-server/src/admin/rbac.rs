...
impl Authenticated {
    // The Err variant is a full axum Response by design (handlers `return` it
    // directly). Boxing it would add a heap allocation on every rejection for no
    // benefit on this rare error path, so the large-move is accepted here.
